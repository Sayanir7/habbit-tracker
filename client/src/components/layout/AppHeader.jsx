...
import { Bell, BrainCircuit, Download, LogIn, LogOut, Moon, Sun, Target } from "lucide-react";
import { IconButton } from "../common/IconButton.jsx";
import { fullDate } from "../../utils/date.js";

export function AppHeader({ today, user, darkMode, isAuthenticated, onAuthOpen, onLogout, onToggleDarkMode, onExport, onReminders, onQuiz, quizActive }) {
  return (
    <header className="min-w-0 rounded-lg border border-stone-200 bg-white/90 p-3 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:flex sm:items-center sm:justify-between sm:p-4">
      ...
      <div className="mt-3 flex w-full min-w-0 items-center gap-2 sm:mt-0 sm:w-auto">
        <button onClick={onQuiz} className={`flex h-10 items-center justify-center gap-2 rounded-lg border px-3 text-sm font-semibold shadow-sm transition ${quizActive ? "border-violet-500 bg-violet-600 text-white" : "border-stone-200 bg-white text-slate-700 hover:border-violet-300 hover:text-violet-700 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"}`}>
          <BrainCircuit size={17} /><span className="hidden sm:inline">Daily quiz</span>
        </button>
